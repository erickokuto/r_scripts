...
africa.shp=readShapePoly("F:/CLIM_DATA/africa_border_shapefile/whole_africa.shp")
...
TheilSenMetrics = foreach(row=1:nrow(spDF_Africa3), .combine=rbind, .verbose=TRUE) %do%
  funTheilSen(spDF_Africa3[row])

rr = apply(spDF_Africa3, 1, funTheilSen)

write.csv(TheilSenMetrics, 
          "/home/erick/Documents/PhD_Projects/Phenology_paper/Phenology_parameters/Phenology_Parameters.csv")

#xyz <- read.csv("/home/erick/Documents/PhD_Projects/Phenology_paper/Phenology_parameters/Phenology_Parameters.csv")
xyz = cbind(coord, TheilSenMetrics)
TheilSenMetrics_STK <- rasterFromXYZ(xyz)
writeRaster(TheilSenMetrics_STK, '/home/erick/Documents/PhD_Projects/Phenology_paper/Phenology_parameters/Phenology_Parameters_STK.tif', bylayer=FALSE)
