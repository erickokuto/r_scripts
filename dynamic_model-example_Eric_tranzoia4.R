...
#Subsetting first 12 months with coordinates in first two columns
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#A random intercept (mu1) which vary in both space and time is added
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
formula <- y ~ 0 + cos.amplitude + sine.amplitude + 
...
res <- inla(formula, family='gaussian', data=inla.stack.data(stk.y),
            control.predictor=list(A=inla.stack.A(stk.y), compute = FALSE),
