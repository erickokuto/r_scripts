...
#Works with calc function from raster package
...
#r <- calc(stk.africa, fun.theilsenslope)
...
#   } else {
  ...
  require(rkt)
  fit.slope = try(rkt(date=x, y=y, correct = F), silent=TRUE) 
  theils.slope = if(class(fit.slope) == "try-error") NA else unclass(round(as.vector(fit.slope$B), digits = 4))
  theils.slope.sl = if(class(fit.slope) == "try-error") NA else unclass(round(as.vector(fit.slope$sl), digits = 4))
