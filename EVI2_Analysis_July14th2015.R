...
plot(stk.Data.evi.raw[[1]])


xx = rnorm (720, mean=1, sd=2)
yy = rnorm (720, mean=0, sd=1)
yy[c(20:25, 40:41, 100:104, 200, 205,300,305:12, 720)] <- NA
y.ts <- ts(yy, frequency=12, start=c(1982,1), end=c(2011,12))
y.ts
y.ts <- na.approx(y.ts)
y.ts2 <- as.vector(y.ts)


ks.est = try(ks.test(xx, y.ts2), silent=TRUE) 
D.statistic=if(class(ks.est)=="try-error") NA else unclass(round(as.vector(ks.est$statistic), digits = 4))
D.statistic.sig=if(class(ks.est)=="try-error") NA  else unclass(round(as.vector(ks.est$p.value), digits = 4))
