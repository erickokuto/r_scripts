...
PhenologyParFunctionWithDataFrame <- function(x) {
  Freq=12; BeginYear=1982; EndYear=2011; 
  size = 360; nyears=30; npars = 14; k = size 
  ...
  if (all(is.na(new.y))) {
    ...
    y.ts <- ts(new.y, frequency=Freq, start=c(BeginYear,1), end=c(EndYear,Freq))
    ...
    mu1=rep(1, times=k)
    idxmu = rep(rep(1:Freq), times=nyears);
    idxsin = rep(rep(1:Freq), times=nyears); 
    idxcos = rep(rep(1:Freq), times=nyears);
    
    #Time varrying covariates
    sin.amplitude <- as.vector(rep(sin(nyears * (2*pi) * (1:k/k)))) ##30 cycles
    cos.amplitude <- as.vector(rep(cos(nyears * (2*pi) * (1:k/k)))) 
    
    require(INLA); require(raster)
    
    #The formula take all these things into account
    formula <- y ~ sin.amplitude + cos.amplitude +
      f(idxsin, sin.amplitude,  model="ar1", cyclic=TRUE, constr=TRUE)+
      f(idxcos, cos.amplitude,  model="ar1", cyclic=TRUE, constr=TRUE)
    
    #inla call:
    result = try(inla(formula,
                      family="gaussian",
                      data=data.frame(y=y, idxsin=idxsin, idxcos=idxcos, 
    ...
    rho.beta1.postmean=if(class(result)=="try-error") NA else unclass(result$summary.hy[3,1])
    rho.beta2.postmean=if(class(result)=="try-error") NA else unclass(result$summary.hy[5,1])
    
    #rho posterior sd
    rho.beta1.postsd=if(class(result)=="try-error") NA else unclass(result$summary.hy[3,2])
    rho.beta2.postsd=if(class(result)=="try-error") NA else unclass(result$summary.hy[5,2])
...
Kolmogorov.Smirnov.Test.Fun <- function(s1, s2) {
  ks_test<- function(v) {
    require(stats); require(bfastSpatial)
    obs <- v[1:split]
    sim <- v[(split+1):(2*split)]
    ks.est <- try(ks.test(obs, sim), silent=TRUE) 
    D.statistic=if(class(ks.est)=="try-error") NA 
    else unclass(round(as.vector(ks.est$statistic), digits = 4))
    D.statistic.sig=if(class(ks.est)=="try-error") NA 
    else unclass(round(as.vector(ks.est$p.value), digits = 4))
  ...
  s <- stack(s1, s2)
  split <- nlayers(s)/2
  ks.result <- mc.calc(s, fun=ks_test, mc.cores = 4)
  return(ks.result)
}


#Two sample Kolmogorov-Smornov Test
# Kolmogorov.Smirnov.Test.Fun <- function(s1, s2, Freq = 12, begin=1982, end=2011) {
#   ks_test<- function(v) {
#     require(stats)
#     obs <- v[1:split]
#     sim <- v[(split+1):(2*split)]
...
#     if (is.na(obs[1])) {
#       
#       return(rep(NA, times = 2))
#       
#     } else {
#       
#       require(stats); require(zoo)
#       obs <- as.vector(zoo::na.approx(ts(obs,
#                                          frequency=Freq, start=c(begin,1), 
#                                          end=c(end,Freq)), na.rm = FALSE))
#       ks.est <- ks.test(obs, sim)  
#       D.statistic= round(as.vector(ks.est$statistic), digits = 4) 
#       D.statistic.sig= round(as.vector(ks.est$p.value), digits = 4) 
#       ks.result = cbind(D.statistic, D.statistic.sig) 
#       return(ks.result)
#       
# }
...
#TheilSen Regression with MannKendall Sig Testing
# Theil_Sen_slope_revised <- function(stk) {
...
#       require(rkt)
#       fit.slope = rkt(date=x, y=y)
#       theils.slope = round(fit.slope$B, digits = 4)
#       theils.slope.sl = round(fit.slope$sl, digits = 4)
...
Theil_Sen_slope_revised <- function(stk) {
  funTheilSen=function(v) {
    ...
    else unclass(round(as.vector(fit.slope$B), digits = 4))
    theils.slope.sl=if(class(fit.slope)=="try-error") NA 
    else unclass(round(as.vector(fit.slope$sl), digits = 4))
    TheilSen.result = cbind(theils.slope, theils.slope.sl)
    return(TheilSen.result)
  }
  require(bfastSpatial); require(raster)
  s <- stack(stk)
  size <- nlayers(s)
  Final.result <- mc.calc(s, fun=funTheilSen, mc.cores = 4)
  return(Final.result)
} 


#test phenology metrics function
# PhenologyParFunctionRevised3 <- function(stk, Freq=12, BeginYear=1981, EndYear=1984) {
#   PhenologyParFunction2 <- function(x) {
#     k = 36; nyears=3; npars = 17
#     new.y=as.vector(x)
#     
#     if (is.na(new.y[1])) {
#       
...
#       #seasonal decomposition of time series by Loess (into trend, season, remainder)
#       require(stats); require(zoo) 
#       y.ts <- ts(new.y, frequency=12, start=c(1982,1), end=c(1984, 12))
#       mySTLdata <- stl(y.ts, s.window="periodic")
#       
#       #coersing the stl object into a dataframe
#       mySTLdata.DF1 <- as.data.frame(mySTLdata$time.series)
...
#   s <- stack(stk)
#   size <- nlayers(s)
#   Final.result <- calc(s, fun=PhenologyParFunction)
#   return(Final.result)
# } 


PhenologyParFunctionRevised2 <- function(stk, Freq=12, BeginYear=1982, EndYear=2011, 
                                         k= 360, nyears=30, npars = 14) {
PhenologyParFunction <- function(v) {
  require(reshape)
    new.y <- melt(v[1:size], na.rm=FALSE)$value
    
    if (all(is.na(new.y[1])))  {
    ...
    #Time varrying covariates
    sin.amplitude <- as.vector(rep(sin(nyears * (2*pi) * (1:k/k)))) ##nyears cycles
    cos.amplitude <- as.vector(rep(cos(nyears * (2*pi) * (1:k/k)))) 
...
# yy2 = rnorm(360)
# yy.ts <- ts(yy2, frequency=12, start=c(1982,1), end=c(2011,12))
# mySTLdata <- stl(yy.ts, s.window="periodic")
# 
# dim(mySTLdata.DF1)
# length(yy.ts)
# 
...
PhenologyParFunctionRevised6 <- function(stk) {
  PhenologyParFunction <- function(v) {
    require(reshape)
    Freq=12; BeginYear=1982; EndYear=2011;
    k= 360; nyears=30; npars = 14;
    ...
    if (is.na(new.y[1]))  {
      ...
      y = new.y
      formula <- y ~ sin.amplitude + cos.amplitude +
        f(idxsin, sin.amplitude,  model="ar1", cyclic=TRUE, constr=FALSE)+
        f(idxcos, cos.amplitude,  model="ar1", cyclic=TRUE, constr=FALSE)
      ...
      #The estimated phenology parameter posterior mean estimates
      beta0=result$summary.fix[1,1]
      sin_beta1=result$summary.fix[2,1]
      cos_beta2=result$summary.fix[3,1]
      Amplitude=sqrt(sin_beta1^2 + cos_beta2^2)
      Phase=asin(sin_beta1/Amplitude)
      
      #The estimated phenology parameter posterior sd estimates
      beta0.sd=result$summary.fix[1,2]
      sin_beta1.sd=result$summary.fix[2,2]
      cos_beta2.sd=result$summary.fix[3,2]
      
      #Model validation parameter estimates
      dic=result$dic$dic
      logCPO=sum(-log(result$cpo$cpo), na.rm=TRUE)
      
      #non transformed spde parameters (theta1) posterior mean
      rho.beta1.postmean=result$summary.hy[3,1]
      rho.beta2.postmean=result$summary.hy[5,1]
      
      #rho posterior sd
      rho.beta1.postsd=result$summary.hy[3,2]
      rho.beta2.postsd=result$summary.hy[5,2]
...
#Works
PhenologyParFunctionRevised5 <- function(stk) {
  PhenologyParFunction <- function(v) {
    require(reshape)
    Freq=12; BeginYear=1982; EndYear=2011;
    k= 360; nyears=30; npars = 14;
...
#Works; but too slow
PhenologyParFunction2 <- function(x, Freq=12, BeginYear=1982, EndYear=2011, 
                                  size = 360, nyears=30, npars = 17) {
  k = size 
  new.y=as.vector(x)
  ...
  if (is.na(new.y[1])) {
    ...
    #non transformed spde parameters (theta1) posterior mean
    rho.beta0.postmean=result$summary.hy[3,1]
    rho.beta1.postmean=result$summary.hy[5,1]
    rho.beta2.postmean=result$summary.hy[7,1]
    
    #rho posterior sd
    rho.beta0.postsd=result$summary.hy[3,2]
    rho.beta1.postsd=result$summary.hy[5,2]
    rho.beta2.postsd=result$summary.hy[7,2]
...
PhenologyParFunction2 <- function(x) {
  Freq=12; BeginYear=1982; EndYear=2011; 
  size = 360; nyears=30; npars = 17; k = size 
  new.y=as.vector(x)
