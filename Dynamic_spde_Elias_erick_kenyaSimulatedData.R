...
stk.y <- inla.stack(data=list(y=as.vector(simdf$ysimNA)), tag='y',
                    A=list(A0, A1, 1),
                    effects=list(i0, i1,
...
write.csv(gofstatistic, "F:/CLIM_DATA/tranzoia_county/other Results/SimData/gofstatistic_SimEvi2Record.csv")
...
write.csv(rangeRF, "F:/CLIM_DATA/tranzoia_county/other Results/SimData/SimspatialRange_sim.csv")
...
varianceRF=round(cbind(prior=sigma2, t(sapply(list(rf0, rf1), function(rrf)
  unlist(inla.zmarginal(rrf$marginals.variance.nominal[[1]],
                        silent=TRUE))))), 3)
write.csv(varianceRF, "F:/CLIM_DATA/tranzoia_county/other Results/SimData/varianceRF_sim.csv")
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
write.csv(kappaRF, "F:/CLIM_DATA/tranzoia_county/other Results/SimData/kappa_sim.csv")


#Plotting the random field parameters
...
fixed_effects = round(cbind(true=mu.beta, res$summary.fix), 4)
write.csv(fixed_effects, "F:/CLIM_DATA/tranzoia_county/other Results/SimData/Simfixed_effects_Data.csv")
...
write.csv(hyperPar_effects_est, "F:/CLIM_DATA/tranzoia_county/other Results/SimData/SimhyperPar_effects_est.csv")


require(reshape) 
index=inla.stack.index(stk.y, 'y')$data
df <- cbind(melt(cbind(kenyadata[, -(1:2)]), na.rm=FALSE),
            SimData=simdf$ysimNA, 
            pred.Sim_mean=round(res$summary.fitted.values[index, "mean"], 4),
            pred.Sim_sd=round(res$summary.fitted.values[index, "sd"], 4),
            x=rep(coo[,1], times=360),
            y=rep(coo[,2], times=360))
df1 <- df[, -2]
head(df1, n=100)

write.csv(df1, "F:/CLIM_DATA/tranzoia_county/other Results/SimData/SimEvi2predTranzoia_SPDE_10km.csv")
