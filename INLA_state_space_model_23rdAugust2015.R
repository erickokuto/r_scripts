...
############################################################################
#   Model One (NULL MODEL: MODEL WITHOUT EFFECTS)
############################################################################
# formula.model1 = test_obs ~ 1
# 
...
############################################################################
#   MODEL TWO (MODEL WITH ONLY TREND EFFECTS)
############################################################################
formula.model1 = test_obs ~ f(trend.effect, model = "rw2", diagonal=1e-8, scale.model=TRUE, constr=TRUE) -1
...
# result.model1$cpo$cpo
#####################################################################
# MODEL THREE (A model with both trend and seasonal effects)
####################################################################
formula.model2 = test_obs ~ f(trend.effect, model = "rw2", 
                              diagonal=1e-8, scale.model=TRUE, constr=TRUE) +
  f(seasonal.effect, model="seasonal", season.length=n.seas, constr=TRUE, 
    diagonal=1e-8) -1
...
########################################################################
# MODEL FOUR (A UNIVARIATE MODEL WITH ALL THE THREE EFFECTS)
##########################################################################
...
round(r1$summary.fitted.values[1:720, 1:2], 4)
