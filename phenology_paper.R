...
require(gimms); require(raster); require(foreach)
...
require(snow); require(MODIS); require(foreach);
require(maptools)
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
require(raster); require(bfastSpatial); require(snow); require(MODIS)

beginCluster(type="SOCK",exclude="MODIS") 
PhenologyMetrics = calc(stk.africa, fun=PhenologyParFunction2) 
writeRaster(PhenologyMetrics, 
  "F:/CLIM_DATA/global/Phenology_parameters/stk.output/spdePhenoParameters.tif")
