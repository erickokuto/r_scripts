...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
n <- nrow(tranzoiadata[, -c(1:2)])
k <- ncol(tranzoiadata[,-c(1:2)])
set.seed(1)
coo <- as.matrix(tranzoiadata[1:n, 1:2])
...
sine.amplitude <- as.vector(rep(sin(31 * (2*pi) * (1:k/k)), each=n)) ### 31 cycles
cos.amplitude <- as.vector(rep(cos(31 * (2*pi) * (1:k/k)), each=n)) ### 31 cycles
...
y<- melt(cbind(tranzoiadata[, -(1:2)]), na.rm=FALSE)$value
...
require(reshape)          
eviObs <- melt(cbind(tranzoiadata[, -(1:2)]), na.rm=FALSE)
evipred <- round(res$summary.fitted.values[index, "mean"], 4)
evi_record <- cbind(coo, eviObs, evipred)
write.csv(evi_record, "F:/CLIM_DATA/kenya_county/Evi2predTranzoia_SPDE.csv")
